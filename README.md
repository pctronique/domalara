...
Pour créer un projet laravel avec docker.
...
Modifier le nom de la base de données dans les fichiers « www/.env » (s'il existe), « .env.local.example » et faire la modification dans le fichier « .env » (si celui-ci existe).
...
```
/www/config/.env.local.example
/www/config/.env
```
Par le nouveau chemin :
```
/www/src/config/.env.local.example
/www/src/config/.env
```
